...
class BacktestingEngine:
    ...
    def __init__(self):
        ...
        self.balances = [] # record balance at new tick arrival
    ...
    def statistics_trade_result(self,df, capital, show_chart=True):
        ...
        self.output(f"百分比最大回撤: {max_ddpercent:,.2f}%")
        self.output(f"收益回撤比：\t{return_drawdown_ratio:,.2f}")
        self.output(f"总成交次数:\t{trade_count}")
        self.output(f"盈利成交次数:\t{win_count}")
        self.output(f"亏损成交次数:\t{loss_count}")
        self.output(f"胜率:\t\t{winning_rate:,.2f}")
        self.output(f"盈亏比:\t\t{win_loss_pnl_ratio:,.2f}")
        self.output(f"平均每笔盈亏:\t{pnl_medio:,.2f}")
        self.output(f"平均持仓小时:\t{duration_medio:,.2f}")
        self.output(f"平均每笔手续费:\t{commission_medio:,.2f}")
        self.output(f"平均每笔滑点:\t{slipping_medio:,.2f}")
        self.output(f"总盈利金额:\t{win_amount:,.2f}")
        self.output(f"盈利交易均值:\t{win_pnl_medio:,.2f}")
        self.output(f"盈利持仓小时:\t{win_duration_medio:,.2f}")
        self.output(f"总亏损金额:\t{loss_amount:,.2f}")
        self.output(f"亏损交易均值:\t{loss_pnl_medio:,.2f}")
        self.output(f"亏损持仓小时:\t{loss_duration_medio:,.2f}")
        if not show_chart:
            return
